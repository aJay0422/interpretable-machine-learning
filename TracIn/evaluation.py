...
for i in range(2, 10):
    # tracin
    net.load_state_dict(torch.load("experiment3/pick large/CNN_CIFAR10_drop{}.pth".format(int(i * 10)), map_location=device))
    _, acc = get_loss_acc(net, testloader, nn.CrossEntropyLoss())
    test_acc_tracin.append(acc)
    # random
...
plt.plot(xrange, test_acc_tracin, '-bo', label="Pick Large")
plt.plot(xrange, test_acc_random, '-ro', label="Pick Small")
